...
int main(int argc, char **argv) {
    ...
    // Initialise database before any table operations
    {
        char *errMsg = nullptr;
        flexql_exec(db, "CREATE DATABASE benchmark", nullptr, nullptr, &errMsg);
        if (errMsg) flexql_free(errMsg);
        errMsg = nullptr;
        flexql_exec(db, "USE benchmark", nullptr, nullptr, &errMsg);
        if (errMsg) flexql_free(errMsg);
        cout << "Database ready\n";
    }
